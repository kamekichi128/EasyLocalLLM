...
**📚 詳細ドキュメント：**
- **[Tools_Design.md](Tools_Design.md)** - 設計ドキュメント（アーキテクチャ、処理フロー、実装詳細）
- **[InputSchema_Examples.md](InputSchema_Examples.md)** - inputSchema の具体例とベストプラクティス
...
public class ToolExample : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        _client.RegisterTool(
            ...
            callback: (Func<int, int, int>)((int a, int b) => a + b)  // ToString() 不要！
        );
    ...
    void Start()
    {
        ...
        StartCoroutine(_client.SendMessageAsync(
            ...
            (response, error) =>
            {
                ...
                // LLM がツールを自動的に呼び出して回答
                Debug.Log($"Assistant: {response.Content}");
                // 例: "The sum of 125 and 378 is 503. However, I don't have real-time capabilities to check the current time. But you can find that out by asking your smart device or a clock nearby."
            },
...
void RegisterDataTools()
{
    ...
    _client.RegisterTool(
        ...
        callback: (Func<string, object>)((string userId) => new
        {
            id = userId,
            name = "John Doe",
            age = 30,
    ...
    _client.RegisterTool(
        ...
        callback: (Func<int, object>)((int count = 10) => new[]
        {
...
public class GameToolsExample : MonoBehaviour
{
    private OllamaClient _client;
    ...
    void Start()
    {
        ...
        // プレイヤーのインベントリをチェックするツール
        _client.RegisterTool(
            name: "check_inventory",
            description: "Check what items the player has in inventory",
            callback: (Func<string, object>)((string itemName = null) =>
            {
                ...
                {
                    // 特定のアイテムの数量を返す
                    int count = _inventory.GetItemCount(itemName);
                    return new { itemName, count, hasItem = count > 0 };
                }
            })
        );
        ...
        // クエスト状態を確認するツール
        _client.RegisterTool(
            name: "check_quest_status",
            description: "Check the status of a quest",
            callback: (Func<string, object>)((string questId) =>
            {
        ...
        _client.RegisterTool(
            ...
            description: "Get the current player position in the game world",
        ...
        // NPC との会話
        StartCoroutine(_client.SendMessageAsync(
            "Do I have enough gold coins? And what's my current quest status?",
            (response, error) =>
            {
                if (error == null)
                {
                    // LLM がツールを使って情報を取得し、回答を生成
                    Debug.Log($"NPC: {response.Content}");
                }
            },
            new ChatRequestOptions { SessionId = "npc-shopkeeper" }
        ));
    }
}
```
